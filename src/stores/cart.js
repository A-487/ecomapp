...
export const decreaseAmount = (id) => {
  //if amount passed in item.svelte pass it here aswell (id,amount)
  cart.update((storeValue) => {
    let item = storeValue.find((item) => item.id === id)
    let cart
    if (item.amount === 1) {
      //if amount passed the check if(amount===1)
...
export const addToCart = (product) => {
  cart.update((storeValue) => {
    const { id, image, title, price } = product //id which comes from product
    let item = storeValue.find((item) => item.id === id)
    let cart
    if (item) {
      ...
      cart = [...storeValue, newItem] //spread out all store values and add up newItem in it
    }
    return cart
  })
}
